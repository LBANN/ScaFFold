...
pieces collected here are the ones that are *not* about either kernel:

* :func:`_env_override` -- the opt-in/opt-out spelling, so both modules accept
  the same words and warn about the same typos.
* :func:`_platform_declines` -- the hardware guard, the one routing condition
  whose failure mode is a *correct* answer at the wrong speed.
* :func:`_replaying_a_forward` -- the "a backward is in flight" probe, with its
  ``is_compiling()`` guard.
* :data:`_functorch_active` -- the ``torch.func`` probe, a routing question and
  not a kernel defect.
...
They are shared rather than copied so that the next module to grow a ladder
inherits them instead of rediscovering them.

Deliberately *not* here: the allowlist of exceptions a rung may fail with.  That
is a property of the kernel behind the rung, closed at that kernel's own
boundary, and belongs next to the rung that uses it.
"""
...
#: comparison would decline the same chip under a different HIP build; the part
#: before the first colon is compared instead.
TUNED_ARCH = "gfx942"

#: ...and the compute-unit count, which is what makes this MI300A rather than
#: ``gfx942`` generally: that arch is the MI300A APU (228 CUs), the MI300X (304)
#: and the MI325X (304).  Several tables are *written in terms of* MI300A's
#: geometry -- ``gather_gemm.candidate_configs`` defaults ``GROUP_M`` to 6
#: because that is MI300A's XCD count, ``triton_group_norm._TUNED``'s largest
#: entry pins a grid of 228, and ``conv3d._policy_declines``'s small-``M`` rule
#: is the shape of the cliff where a GEMM stops filling 228 CUs.  Elsewhere
#: those are not bad numbers, just answers about a different machine.
#:
#: The CU count also declines a *partitioned* MI300A, which an arch-only test
#: would silently accept and which is genuinely mistuned: CPX mode presents one
#: logical device per XCD, so both the 228 and the 6 above become fiction while
#: ``gcnArchName`` stays exactly the same.
TUNED_CU_COUNT = 228

#: Both ladders' opt-in switches, named together in the one message a user on
#: another device gets: the verdict is a fact about the *node* rather than about
#: either kernel, so someone who has just discovered the fast path is off should
#: not have to find the second switch separately.
...
def _device_fingerprint(index):
    ...
    Split out from :func:`_platform_verdict` as the seam the tests replace:
    every interesting branch of this guard is one this node cannot take -- an
    MI300X, a partitioned MI300A, an NVIDIA device, a driver that will not
    answer -- so the query has to be injectable, and keeping it (and nothing
    else) behind one function lets a test substitute a tuple and exercise the
    real decision, caching and message rather than a copy of them.

    ``gcnArchName`` exists only on a ROCm build, so an NVIDIA device is
    described as an empty arch and declines through the same clause an untuned
    AMD one does.  That is right for a reason beyond tuning: the kernels' launch
    constraints are MFMA constraints (``gather_gemm._MFMA_KDIM``,
    ``matrix_instr_nonkdim``) and mean nothing on a device with no MFMA.
    """
...
def _platform_verdict(device):
    ...
    Returns ``(ok, description)``, computed once per device index for the life
    of the process: it is asked on the routing path of every convolution and
    every GroupNorm, and ``get_device_properties`` is a driver query rather than
    an attribute read.  Computed lazily rather than at import because a CPU-only
    run must not initialize the GPU at all; the callers only reach here after
    ``is_cuda``, so by then torch's CUDA state is already up.

    Keyed per device index rather than per process because the question is
    whether the kernel about to launch will run on the machine its launch
    configuration was chosen for, which is a property of the device the tensor
    is on.  ScaFFold pins one rank per GPU, so in production this dictionary
    holds exactly one entry; on a node that exposes two different GPUs a
    process-wide answer taken from device 0 would be wrong on one of them.
...
def _reset_platform_cache():
    ...
    For tests only.  The cache above is process-global, so without this the
    first test to ask a question fixes the answer for every later one, and a
    suite that checks the decline path and then the accept path would pass while
    testing the first one twice.
    """
...
def _platform_declines(device, override):
    ...
    This protects against silent mistuning, not a crash.  Every number that
    decides how these kernels launch -- the convolution tile tables and
    ``matrix_instr_nonkdim``/``kpack``/``waves_per_eu`` choices in
    ``triton_conv3d``, and the entries written in terms of MI300A's geometry
    listed at :data:`TUNED_CU_COUNT` -- was raced on one MI300A.  Run somewhere
    else they are answers to a question about a different machine, and nothing
    ...
    * every ``is_supported*`` predicate reads shape, dtype, layout and stride,
    ...
    So the guard is a preference, not a correctness condition, and the code says
    so by letting an explicit opt-in through.  ``override`` is the caller's
    ...
    or called ``set_conv_triton_enabled(True)`` and is asserting a judgement
    about their own hardware, the development case this override exists for.
    ``False`` never reaches here -- the callers decline on it first.

    Both branches are loud, once per device; see :func:`_warn_platform_decline`
    and :func:`_warn_platform_override` for what each owes the reader.
    """
...
def _warn_platform_decline(index, described):
    ...
    Once per device index -- not once per call, which at dozens of routed
    operations a step would be a log line every few milliseconds, and not
    silence, which is the state this guard exists to end.  The message names the
    device it found, the device it wanted, and both switches
    (:data:`_OVERRIDE_SWITCHES`).

    ``is_compiling()`` for the reason :func:`_warn_rung_failure` documents:
    Dynamo cannot trace a ``logging.Logger`` method.
    """
...
def _warn_platform_override(index, described):
    ...
    against was measured on the device this run is *not* on, so a number
    produced under this override is not comparable with any of them, and a log
    line is the only place that fact can be recovered from afterwards.
    """
...
def _replaying_a_forward():
    ...
    forward whose saved tensors are already held.  Answering there on a
    different rung than the original forward used is not a fallback but a
    corruption -- the rungs do not save interchangeable tensors, so the
    recompute's saved set no longer matches the graph node that will consume it.
    For GroupNorm that shows up as ``CheckpointError: Recomputed values ... have
    different metadata``, or on some shapes a GPU memory fault; for convolution
    under DistConv it is worse, because the two rungs save tensors with
    *identical* shape, dtype and device and differ only in whether slot 0 is the
    ``DCTensor`` wrapper or its inner tensor -- which is precisely what
    ``_default_meta_extractor`` does not compare, so the substitution succeeds
    and fails later as an ``AttributeError`` from inside DistConv.  See the
    callers.
    ...
    Dynamo folds ``is_compiling()`` to ``True`` at trace time, leaving ``False``
    here as a constant -- also the right answer, since tracing is not replaying
    and the recompute this guards against runs with Dynamo disabled anyway
...
def _run_local(input, distconv, kernel):
    ...
    public spelling of the latter; there is no public unwrap) rather than a bare
    ``input._tensor`` read: DistConv's own dispatch may read ``_tensor``
    directly because it runs *below* autograd, while this runs above it and a
    bare read would sever the graph back to the producing convolution.

    This does *not* consult the parallel strategy.  Whether running the kernel
    on the local shard alone is the same computation DistConv's dispatch would
    have performed is the caller's question, and the answer differs by operator
    -- see :class:`~ScaFFold.unet.conv3d.FastConv3d`, where it is only true when
    nothing is actually sharded.
    """
    if distconv is None:
        return kernel(input)
...
def _warn_rung_failure(what, error, fallback, env_var):
    ...
    the caller for whom the fallback matters most, since the failure it is
    reacting to is usually a compile failure.  ``is_compiling()`` folds to
    ``True`` at trace time, so the call below is dead code inside a traced
    region and the fallback traces cleanly.  The latch itself is a global
    assignment, which Dynamo does replay, so the fallback is still recorded;
    only this message is dropped.
    """
    if torch.compiler.is_compiling():
        return
    logger.warning(
...
def kernel_selection(model):
    ...
    This reads a latch, not a decision.  ``_triton_ok`` is set on the first call
    a rung actually answers, so a module that has not run yet reports ``Native``
    -- truthfully, in that it has used no other kernel, but misleadingly, in
    that it has used none at all.  Callers must therefore run at least one
    forward first; :meth:`PyTorchTrainer._log_kernel_selection` does.
