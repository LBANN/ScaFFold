...
voxel*, and a wrong voxel holds a plausible number: swap a tap, drop a boundary
compare or transpose two spatial strides and the result is still smooth, still
the right magnitude, and still passes ``allclose``.  So the primary standard
here is bitwise, made attainable by drawing operands from ``{-1, 0, 1}`` --
every product is exact and every partial sum a small integer, so the reference
and the kernel must agree exactly or the kernel is wrong.
:func:`test_bitwise_standard_rejects_a_shifted_gather` proves that standard has
teeth rather than being vacuously satisfied.

The tolerance-based tests are the second line: they hold at real magnitudes and
real reduction lengths, where exactness is not available.
...
#:
#: Selecting by ``volume * channels`` has a non-obvious consequence: the
#: problems that survive the filter are the *widest* ones -- 256->512 up to
#: 1024->1024, K = 6912 to 27648 -- because those are the ones ScaFFold runs at
#: a small spatial extent.  So this list is precisely the regime where the
#: forward's reduction is longest, which is where exactness is hardest.  See
...
def test_validate_rejects_the_silent_mfma_failures():
    ...
    ``BLOCK_K=8`` at ``nonkdim=16`` and an illegal ``nonkdim=64`` both compile,
    run and return correct results with the dot lowered to vector FMA.  Nothing
    raises and nothing warns, so a config generator that merely ranked them last
    would still feed meaningless entries into a best-of sweep -- which is why
    :meth:`ConvConfig.validate` refuses.
    """
    bf16 = torch.bfloat16
...
def test_validate_rejects_a_group_m_that_faults_the_gpu():
    """``GROUP_M`` faults the GPU rather than falling back to FMA.

    Its failure mode is not the silent FMA fallback the other fields have, which
    is why it is stated apart from them: the swizzle computes
    ``width = GROUP_M * grid_n`` and then ``pid // width``, so ``GROUP_M = 0``
    divides by zero inside the kernel -- on gfx942 that is a garbage ``pid_m``
    and a memory access fault, not a trap -- and ``GROUP_M = -3`` reaches the
    kernel just as far.
...
def test_the_index_width_decision_covers_every_operand_including_the_weight():
    ...
    The weight has to be one of the operands it looks at: past ``2**31 - 1``
    elements of ``taps * Cin * Cout`` the int32 offset wraps negative and the
    GPU faults, while ``is_supported`` still returns ``True``.  Meta tensors
    carry a ``numel`` and no storage, so this end of it costs nothing to state.
    """
...
def test_block_k_constraint_follows_the_intrinsic_not_the_tile():
    ...
    at 16, and ``BLOCK_K = 16`` is legal at both -- the constraint is not a
    property of the block size alone.
    """
    ok32 = ConvConfig(
...
def test_default_config_fits_in_lds_in_every_dtype(dtype):
    ...
    A ``128x128x128`` tile is 64 KiB in bf16 and 128 KiB in fp32, so a heuristic
    that picks it for ``Cin >= 512`` raises ``OutOfResources`` on any wide fp32
    convolution.  ``more_determinism`` runs the model in fp32, so a real ScaFFold
    configuration reaches that.
...
def test_is_supported_never_raises_on_an_argument_it_cannot_parse():
    ...
    nor iterable and ``ValueError`` for a wrong-length one, so both have to be
    caught for the predicate to be total.
    """
    x = torch.empty((1, 8, 4, 4, 4), device="cuda", dtype=torch.bfloat16)
    w = torch.empty((8, 8, 3, 3, 3), device="cuda", dtype=torch.bfloat16)
    assert is_supported(x, w, padding=1)
...
def test_is_supported_declines_a_bias_torch_itself_rejects():
    ...
    Neither is a property of the bias, so both failures are silent: a short bias
    reads whatever is in memory past its end, and a stride-2 view of the right
    length applies ``[0,1,2,...]`` where the caller passed ``[0,2,4,...]``.
    ``torch.conv3d`` refuses the first outright; this predicate refuses both.
    """
    bf16 = torch.bfloat16
...
def test_the_forward_gate_alone_is_a_trap_for_a_caller_that_differentiates():
    ...
    the right contraction at unit stride.  The trap is that a training caller
    who asks :func:`is_supported`, gets ``True`` and builds a graph node finds
    out at ``backward()``, where its own MIOpen fallback is no longer reachable
    because the node is already in the graph.

    So this pins both halves: the trap remains at the direction gates (they
...
def test_is_supported_all_is_exactly_the_three_gates_conjoined():
    ...
    Two things could rot without failing loudly.  The gradient reaches the two
    backward predicates as a metadata-only stand-in -- a one-element allocation
    expanded to the output shape -- which is sound only while they read metadata
    and nothing else, so it is compared against the answer a *real* gradient
    gets.  And the output shape is computed here rather than by the caller, so a
    wrong one would be a gate answering about a different problem.
    """
...
def test_is_supported_declines_degenerate_extents():
    """Three shapes where the kernel returns something ``torch.conv3d`` does not.

    Each clears the "every output voxel must exist" test and then diverges,
    which is the asymmetry the predicate exists to prevent: the MIOpen fallback
    raises on all three and the Triton path silently would not.
...
def test_bitwise_standard_rejects_a_shifted_gather():
    ...
    ``{-1,0,1}`` operands happen to make everything agree.  So the kernel's
    output is compared against a reference computed for a *deliberately wrong*
    gather, and a mismatch is required.
    """
...
def test_corpus_shapes_match_bitwise(problem: ConvProblem):
    ...
    Exactness has to come from the operands here, not from the shape.  A dense
    ``{-1,0,1}`` draw is exact only while the realized sums stay inside the
    mantissa, and bf16 holds integers only to 256, while the forward reduces
    over ``Cin * taps`` -- 27 648 terms at ``Cin = 1024`` -- whatever the
    volume, so the restatement at ``6x7x8`` the two backward suites use buys
    nothing.  The activations are thinned to :func:`reference.exact_density`
    instead, leaving the shape, the channel widths and the weight as they are:
    it is the gather that is under test, not the arithmetic.  A thinned draw is
    also how one accidentally builds a test that compares zeros against zeros,
    so two things are asserted rather than assumed -- that the answer is mostly
    nonzero, and that the comparison still rejects a one-voxel shift.

    This test does not skip.  If the draw is ever not exact, that is a fact
    worth failing on rather than stepping around.
    """
...
def test_the_bitwise_corpus_is_not_entirely_skipped():
    ...
    name, because a suite that skips every real-shape case still reports a wall
    of passes.  The shape of the guard differs from theirs, because
    :func:`test_corpus_shapes_match_bitwise` has no skip branch left: what needs
    pinning is not "enough cases are representable" but the two ways the test
    could still stop meaning anything -- the parametrization collapsing to
...
def test_every_config_gives_the_same_answer(problem: ConvProblem):
    ...
    when ``BLOCK_M`` divides ``OUT_W`` and wrong when it does not, or a
    ``BLOCK_K`` remainder only exercised when ``Cin`` is not a multiple of the
    tile.  Sweeping the whole candidate list against a bitwise reference tests
    the *tuning surface* rather than one point on it, which matters because the
    tuned table is free to pick any of them.

    All of ``EDGE``, not a prefix of it: a prefix leaves ``batched`` (the only
    ``n > 1`` case), ``kernel_aniso``, ``smaller_than_kernel``, ``unpadded``,
    ``pointwise`` and both non-bf16 dtypes at the *default* config alone.  The
    dtype matters in its own right: it moves the MFMA intrinsic's reduction
    depth and therefore which ``BLOCK_K`` values are legal.
    """
...
def test_out_buffer_is_written_in_place_and_is_validated():
    """An unvalidated ``out=`` is an out-of-bounds device write.

    The grid is sized from the problem rather than from the buffer, so an
    undersized ``out=`` writes past the end with no error and no fault,
    surviving only while the allocator slab happens to be bigger.  An NCDHW
    buffer is the other half: the store addressing writes NDHWC strides into it
    and returns a scrambled answer at full speed.

    Handing a preallocated gradient buffer to the backward is precisely what the
    ``nn.Module`` adapter does, so this is the parameter that most needs the
    check.
    """
...
def test_the_output_is_allocated_directly_in_channels_last():
    ...
    the whole tensor, on a path a training step takes once per convolution.  The
    copy is invisible in the result, so what pins it is the peak allocation: the
    wrong form needs two output-sized buffers live at once, the right form needs
    one.
    """
    x = torch.randn(
...
def test_the_layout_conversion_is_a_no_op_only_where_stride_c_is_moot():
    ...
    That is safe for a reason outside this code: every ambiguous shape either
    has ``stride(1) == 1`` outright (all three spatial extents are 1) or has
    ``Cin == 1``, which makes the channel stride unobservable because the only
    channel index the kernel dereferences is 0.  It is a property of PyTorch's
    predicate rather than of ours, so the enumeration over ``{1,2,3}^5`` pins it
...
def test_hoisted_weight_transform_is_validated():
    ...
    result.  That is a live hazard rather than a "you asked for it": the
    transform exists to be cached across calls, and a cache keyed on the
    parameter's version is exactly the thing that goes stale.
    """
...
def test_every_weight_layout_gives_the_same_answer():
    ...
    a gathered tile is much slower when *neither* channel axis is unit-stride,
    ...
    The RSCK-strided case is the one that most needs a test: PyTorch's shape
    over this kernel's storage order, which is what an integration allocates to
    make the B tile contiguous, and the case where ``to_rsck`` is a no-op and
    ``weight`` and ``weight_rsck`` are the same tensor -- so also where a mix-up
    between them would hide.
    """
...
def test_indices_beyond_int32_are_addressed_correctly():
    ...
    Unsharded scale 8 is ``1 x 128 x 258^3 = 2.20e9`` elements, past the
    ``2**31 - 1`` an int32 offset can address; it is also where MIOpen itself
    asserts and where the buffer-load fast path is lost, because
    ``is_within_2gb`` reads the whole storage.  Losing buffer loads is a
    performance question; getting the *index* wrong is a correctness one, and
    only a tensor this size asks it.
...
def test_indices_beyond_int32_are_addressed_correctly_with_taps_and_padding():
    ...
    Same ``1 x 128 x 258^3`` activation (2.20e9 elements, unsharded scale 8), at
    ``k=3, padding=1``.

    One tap, the last one, so that ``tap_off`` is at its maximum and the far
    corner of the output reads the far corner of the input -- where a truncated
    offset aliases back towards the start.
    """
    free, _ = torch.cuda.mem_get_info()
    if free < 12 << 30:
        pytest.skip("needs ~12 GiB free")
...
def test_a_weight_beyond_int32_is_addressed_correctly():
    """``taps * Cin * Cout`` over ``2**31``, where an int32 offset faults.

    Not reachable from this model -- its widest weight is far below the limit --
    ...
    What has to overflow is ``dij * stride_wt + offs_k * stride_wc``: ``offs_n``
    is a *second* ``addptr``, sign-extended on its own, so excess put there
    passes even against an int32 weight offset.  The quantity to push past
    ``2**31`` is therefore ``taps*Cin*Cout - Cout``.
